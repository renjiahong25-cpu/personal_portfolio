...
export default defineConfig(({ mode }) => ({
  define: {
    __WEB_AGENT_DEV__: JSON.stringify(mode === 'development'),
  },
  plugins: [
    {
    ...
    },
    {
      ...
      transform(source, id) {
        if (!id.endsWith('.mjs')) return undefined
        if (!source.startsWith('#!/usr/bin/env node')) return undefined
        return `// #!/usr/bin/env node\n${source.slice('#!/usr/bin/env node\n'.length)}`
      },
    },
    {
