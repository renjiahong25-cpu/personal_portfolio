...
import type { GroupChat, GroupRole, WebAgentStore, RuntimeFrameBinding } from '../group/types'

function createStoreWithReadyRole(): WebAgentStore {
...
describe('background message handlers', () => {
  ...
  it('skips the Web Agent persona when sending a first prompt to a ChatGPT GPTs role', async () => {
    vi.resetModules()
    vi.doMock('./storeAccess', async importOriginal => {
      const actual = await importOriginal<typeof import('./storeAccess')>()
      return {
  ...
  it('streams external model replies into a pending assistant message', async () => {
    ...
    const broadcasts: WebAgentStore[] = []
    let messageIdSeq = 0
    const routes = createMessageHandlers({
      broadcastStoreUpdated: vi.fn(async (store: WebAgentStore) => {
    ...
    const sendRoute = routes.find(route => route.type === 'GROUP_MESSAGE_SEND')
    const response = await sendRoute?.handler({ type: 'GROUP_MESSAGE_SEND', chatId: 'chat-1', raw: '@all 流式回答' }, {}) as { store: WebAgentStore } | undefined
  ...
  it('retries external model failures before content starts and keeps one assistant placeholder', async () => {
    ...
    const sendRoute = routes.find(route => route.type === 'GROUP_MESSAGE_SEND')
    const response = await sendRoute?.handler({ type: 'GROUP_MESSAGE_SEND', chatId: 'chat-1', raw: '@all 用 API 回答' }, {}) as { store: WebAgentStore }
  ...
  it('stops a site role gracefully when the frame binding is missing (e.g. extension restart)', async () => {
    vi.resetModules()
    const startingStore = createStoreWithReadyRole()
    startingStore.rolesById['role-1'].status = 'thinking'
    startingStore.rolesById['role-1'].lastPromptMessageId = 'msg-1'
    ...
    const stopRoute = routes.find(route => route.type === 'GROUP_ROLE_STOP_REPLY')
    const response = await stopRoute?.handler({ type: 'GROUP_ROLE_STOP_REPLY', chatId: 'chat-1', roleId: 'role-1' }, {}) as { store: WebAgentStore }

    expect(response).toMatchObject({ ok: true, messageId: 'msg-1' })
    expect(getByRole).toHaveBeenCalledWith('chat-1', 'role-1')
    expect(sendRoleMessage).not.toHaveBeenCalled()
  ...
  it('keeps a stopped external stream in its existing assistant message', async () => {
    ...
    const sendRoute = routes.find(route => route.type === 'GROUP_MESSAGE_SEND')
    const stopRoute = routes.find(route => route.type === 'GROUP_ROLE_STOP_REPLY')
    const sendPromise = sendRoute?.handler({ type: 'GROUP_MESSAGE_SEND', chatId: 'chat-1', raw: '@all 流式回答' }, {}) as Promise<{ store: WebAgentStore }>
    await firstChunkStored
    const stopResponse = await stopRoute?.handler({ type: 'GROUP_ROLE_STOP_REPLY', chatId: 'chat-1', roleId: 'role-1' }, {}) as { store: WebAgentStore }
    ...
    const response = responseOrTimeout as { store: WebAgentStore }
    expect(response.store.messagesById['msg-2']).toMatchObject({
      type: 'assistant',
      content: '第一段',
      status: 'received',
  ...
  it('retries external model replies by discarding the selected assistant message and streaming again', async () => {
    ...
    const response = await retryRoute?.handler({ type: 'GROUP_ROLE_RETRY_REPLY', chatId: 'chat-1', roleId: 'role-1', messageId: 'msg-old-reply' }, {}) as { ok: boolean; store: WebAgentStore }
  ...
  it('routes TEAM_SITE_STATUS_UPDATE into the site status registry', async () => {
    vi.resetModules()
    const { createMessageHandlers } = await import('./messageHandlers')
    const { createSiteStatusRegistry } = await import('./siteStatus')
    ...
    const statusRoute = routes.find(route => route.type === 'TEAM_SITE_STATUS_UPDATE')
    const response = await statusRoute?.handler({ type: 'TEAM_SITE_STATUS_UPDATE', siteId: 'deepseek', status: 'unauthorized', detail: '未登录' }, {})

    expect(response).toEqual({ ok: true })
    expect(siteStatus.get('deepseek')).toEqual({ status: 'unauthorized', detail: '未登录', timestamp: 100 })
  })

  it('rejects GROUP_MESSAGE_SEND with a login hint when the target site is unauthorized', async () => {
    vi.resetModules()
    let draftStore: WebAgentStore | undefined
    vi.doMock('./storeAccess', async importOriginal => {
      const actual = await importOriginal<typeof import('./storeAccess')>()
      return {
        ...
        mutateStore: vi.fn(async (mutator: (store: WebAgentStore) => unknown) => {
          const store = createStoreWithReadyRole()
          store.rolesById['role-1'].chatSite = 'deepseek'
          draftStore = store
          const result = await mutator(store)
          return { store, result }
        }),
    ...
    const { createSiteStatusRegistry } = await import('./siteStatus')
    const siteStatus = createSiteStatusRegistry()
    siteStatus.set('deepseek', 'unauthorized', '未登录', 99)
    const binding: RuntimeFrameBinding = { chatId: 'chat-1', roleId: 'role-1', tabId: 101, frameId: 7, ready: true, lastSeenAt: 0 }
    const routes = createMessageHandlers({
      broadcastStoreUpdated: vi.fn(),
      getChatStatusFromRoles: () => 'ready',
    ...
    await expect(sendRoute?.handler({ type: 'GROUP_MESSAGE_SEND', chatId: 'chat-1', raw: '@all 帮我评审一下' }, {})).rejects.toThrow(/站点未登录/)

    expect(Object.keys(draftStore?.messagesById ?? {})).toHaveLength(0)
    expect(draftStore?.rolesById['role-1'].status).toBe('ready')
  })
})
