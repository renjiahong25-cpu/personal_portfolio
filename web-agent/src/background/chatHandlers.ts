...
import type { GroupChat, GroupMessage, GroupRole, WebAgentStore, OrchestrationFlow, OrchestrationStage, RoomMode } from '../group/types'
...
function createChat(store: WebAgentStore, message: RuntimeMessage, deps: ChatHandlersDependencies): GroupChat {
  const timestamp = deps.now()
...
export function duplicateChat(store: WebAgentStore, sourceChatId: unknown, deps: ChatHandlersDependencies): { chat: GroupChat; roles: GroupRole[] } {
  const timestamp = deps.now()
...
function duplicatedChatName(store: WebAgentStore, sourceName: string): string {
...
function markChatRead(store: WebAgentStore, chat: GroupChat): void {
