...
import type { BackgroundToRoleMessage, SiteStatus } from '../group/runtimeProtocol'
import type { ExternalModelConfig, GroupChat, GroupMessage, GroupRole, MessageReference, WebAgentStore, RuntimeFrameBinding } from '../group/types'
...
import type { SiteStatusRegistry } from './siteStatus'
...
const VALID_SITE_STATUSES = new Set<SiteStatus>(['ready', 'generating', 'error', 'blocked', 'unauthorized'])

const CHAT_SITE_DISPLAY_NAME: Record<string, string> = {
  deepseek: 'DeepSeek',
  doubao: '豆包',
  chatgpt: 'ChatGPT',
  gemini: 'Gemini',
...
function siteDisplayName(chatSite: string | undefined): string {
  return CHAT_SITE_DISPLAY_NAME[chatSite ?? ''] ?? chatSite ?? '目标站点'
}

function isSiteStatusUnauthorized(role: GroupRole, registry: SiteStatusRegistry | undefined): boolean {
  return registry?.get(role.chatSite ?? '')?.status === 'unauthorized'
}
...
export function createMessageHandlers(deps: MessageHandlersDependencies): BackgroundMessageRoute[] {
  ...
  const handleMessageSend = async (message: RuntimeMessage) => {
    ...
    const messageImage = isRecord(message.image) && typeof message.image.data === 'string' && typeof message.image.mimeType === 'string'
      ? { data: message.image.data, mimeType: message.image.mimeType }
      : undefined
    ...
    const { store, result } = await mutateStore(store => {
      ...
      const unavailable = targetRoles.filter(role => {
        ...
        if (isSiteStatusUnauthorized(role, deps.siteStatus)) return true
      ...
      if (unavailable.length > 0 && deliverableTargetRoles.length === 0) {
        deps.log.warn('message-send:unavailable-targets', {
          ...
          targets: unavailable.map(role => ({
            ...
            loginRequired: isSiteStatusUnauthorized(role, deps.siteStatus),
          })),
        })
        const loginBlocked = unavailable.filter(role => !isExternalModelRole(role) && isSiteStatusUnauthorized(role, deps.siteStatus))
        if (loginBlocked.length > 0) {
          throw new Error(`站点未登录，请先在 ${[...new Set(loginBlocked.map(role => siteDisplayName(role.chatSite)))].join('、')} 页面完成登录后重新发送。`)
        }
      ...
      for (const role of deliverableTargetRoles) {
        const prepared = prepareRolePromptDelivery({ store, chat, role, userMessage, roles, messages, reference, image: messageImage, timestamp, newId: deps.newId, runtimeFrames: deps.runtimeFrames })
  ...
  const handleRoleStopReply = async (message: RuntimeMessage) => {
    ...
    if (active.isExternal) {
      ...
      if (binding?.ready) {
        ...
        deps.log.warn('role-stop-reply:no-binding', { chatId, roleId, messageId: active.messageId })
      }
    }
  ...
  const handleMessageResyncReply = async (message: RuntimeMessage) => {
    ...
    if (isRecord(response) && isRecord(response.store)) {
      ...
      return { ok: true, store: response.store as unknown as WebAgentStore, message: response.message, messageId }
    }
  ...
  const handleSiteStatusUpdate = async (message: RuntimeMessage) => {
    const siteId = requireString(message.siteId, '缺少站点 ID')
    const status = message.status
    if (typeof status !== 'string' || !VALID_SITE_STATUSES.has(status as SiteStatus)) {
      throw new Error(`未知的站点状态：${String(status)}`)
    }
    const timestamp = deps.now()
    deps.siteStatus?.set(siteId, status as SiteStatus, typeof message.detail === 'string' ? message.detail : undefined, timestamp)
    deps.log.debug('site-status:update', { siteId, status, detail: message.detail })
    return { ok: true }
  }
...
function getRoleReplyHistory(store: WebAgentStore, chat: GroupChat, roleId: string, limit = 100): string[] {
...
function staleReplyReason(store: WebAgentStore, chat: GroupChat, role: GroupRole, promptMessageId: string | undefined, replyAttemptId?: string): string | undefined {
...
function markChatHasNewMessage(store: WebAgentStore, chat: GroupChat): void {
...
function hasAssistantReplyAfterPrompt(store: WebAgentStore, chat: GroupChat, role: GroupRole, promptMessageId: string): boolean {
...
async function sendExternalModelDelivery(deps: MessageHandlersDependencies, client: ExternalModelClient, runs: ExternalModelRunRegistry, delivery: ExternalPromptDelivery): Promise<WebAgentStore | undefined> {
...
): Promise<{ ignored: true; reason: string; store: WebAgentStore } | { ignored: false; replyMessageId: string; store: WebAgentStore }> {
  const timestamp = deps.now()
  const { store, result } = await mutateStore(store => {
...
async function updateExternalAssistantContent(
  ...
  content: string,
): Promise<{ ignored: true; reason: string; store: WebAgentStore } | { ignored: false; store: WebAgentStore }> {
  const timestamp = deps.now()
  const { store, result } = await mutateStore(store => {
    const chat = requireChat(store, delivery.chatId)
    const role = requireRole(store, chat.id, delivery.roleId)
...
function finalizeActiveExternalAssistantReply(store: WebAgentStore, chat: GroupChat, role: GroupRole, promptMessageId: string): boolean {
...
function findPendingAssistantAfterPrompt(store: WebAgentStore, chat: GroupChat, role: GroupRole, promptMessageId: string): AssistantGroupMessage | undefined {
  const promptIndex = chat.messageIds.indexOf(promptMessageId)
...
function findFailedOrPendingAssistantAfterPrompt(store: WebAgentStore, chat: GroupChat, role: GroupRole, promptMessageId: string): AssistantGroupMessage | undefined {
  const promptIndex = chat.messageIds.indexOf(promptMessageId)
  if (promptIndex < 0) return undefined
...
function roleFailureReplyContent(reason: string): string {
  ...
  if (normalized === 'site_login_required') return '⚠️ 站点未登录，回复未发出。请先在站点页面完成登录，登录后会自动恢复，重新发送即重试。'
...
function findLatestPendingRetryMessage(store: WebAgentStore, chat: GroupChat, role: GroupRole): GroupMessage | undefined {
...
function findPromptBeforeAssistant(store: WebAgentStore, chat: GroupChat, role: GroupRole, assistantMessageId: string): GroupMessage | undefined {
...
function discardMessage(store: WebAgentStore, chat: GroupChat, messageId: string): void {
...
function resolveReference(store: WebAgentStore, chat: GroupChat, raw: unknown, newId: (prefix: string) => string): MessageReference | undefined {
...
function requireNoteContent(value: unknown): NonNullable<WebAgentStore['globalNote']> {
  if (!isRecord(value) || typeof value.type !== 'string') throw new Error('笔记内容格式无效')
  return value as NonNullable<WebAgentStore['globalNote']>
}
