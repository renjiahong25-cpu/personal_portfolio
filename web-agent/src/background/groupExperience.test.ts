...
import type { GroupChat, GroupMessage, GroupRole, WebAgentStore, OrchestrationFlow, RoleTemplate } from '../group/types'
...
type BackgroundHarness = {
  ...
  getStore: () => Promise<WebAgentStore>
  invoke: (message: RuntimeMessage, sender?: MessageSender) => Promise<unknown>
}

async function setupBackground(initialStore?: WebAgentStore): Promise<BackgroundHarness> {
  vi.resetModules()
...
describe('background group chat experience handlers', () => {
  it('validates GROUP_ROLES_CREATE_BATCH as a whole batch and keeps temporary people out of the library', async () => {
    ...
    }) as { ok: boolean; roles: GroupRole[]; store: WebAgentStore }
  ...
  it('protects used role templates from deletion and deletes unused templates', async () => {
    ...
    const deleted = await harness.invoke({ type: 'ROLE_TEMPLATE_DELETE', templateId: 'template-unused' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('saves rich note documents for global and chat scopes', async () => {
    ...
    const globalSaved = await harness.invoke({ type: 'GROUP_NOTE_SAVE', scope: 'global', content: globalNote }) as { ok: boolean; store: WebAgentStore }
    const chatSaved = await harness.invoke({ type: 'GROUP_NOTE_SAVE', scope: 'chat', chatId: 'chat-1', content: chatNote }) as { ok: boolean; store: WebAgentStore }
  ...
  it('stores message highlights without changing the original message content', async () => {
    ...
    const response = await harness.invoke({
      ...
      endOffset: 7,
      color: '#7dd3fc',
    }) as { ok: boolean; store: WebAgentStore }
  ...
  it('still sends to available people when another targeted person is unavailable', async () => {
    ...
    await harness.invoke({ type: 'TEAM_FRAME_ROLE_READY', chatId: 'chat-1', roleId: 'role-1', hostTabId: 900 }, { tab: { id: 101 } as chrome.tabs.Tab, frameId: 7, url: 'https://gemini.google.com/app/test' })
    const response = await harness.invoke({ type: 'GROUP_MESSAGE_SEND', chatId: 'chat-1', raw: '@all 请评估这个方案' }) as { ok: boolean; message: { id: string }; store: WebAgentStore }
  ...
  it('advances context cursor only to the acknowledged prompt message', async () => {
    ...
    const response = await harness.invoke({ type: 'TEAM_SEND_ACK', chatId: 'chat-1', roleId: 'role-1', messageId: 'msg-1' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('stores explicit mentions for display while sending the cleaned message content', async () => {
    ...
    await harness.invoke({ type: 'TEAM_FRAME_ROLE_READY', chatId: 'chat-1', roleId: 'role-1', hostTabId: 900 }, { tab: { id: 101 } as chrome.tabs.Tab, frameId: 7, url: 'https://gemini.google.com/app/test' })
    const response = await harness.invoke({ type: 'GROUP_MESSAGE_SEND', chatId: 'chat-1', raw: '@工程师 请评估这个方案' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('records no-mention ordinary chat messages without triggering AI', async () => {
    ...
    const response = await harness.invoke({ type: 'GROUP_MESSAGE_SEND', chatId: 'chat-1', raw: '先记录这个背景' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('renames chats, marks background replies as new, and clears new-message state when read', async () => {
    ...
    const reply = await harness.invoke({ type: 'TEAM_ROLE_REPLY', chatId: 'chat-2', roleId: 'role-2', content: '后台回复', messageId: 'msg-user' }) as { ok: boolean; store: WebAgentStore }
    ...
    const markedRead = await harness.invoke({ type: 'GROUP_CHAT_MARK_READ', chatId: 'chat-2' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('duplicates a chat with copied roles but without messages or Gemini conversation bindings', async () => {
    ...
    const duplicated = await harness.invoke({ type: 'GROUP_CHAT_DUPLICATE', chatId: 'chat-1' }) as { ok: boolean; chat: GroupChat; roles: GroupRole[]; store: WebAgentStore }
  ...
  it('deletes a chat with its roles, messages, read state, and runtime bindings', async () => {
    ...
    await harness.invoke({ type: 'TEAM_FRAME_ROLE_READY', chatId: 'chat-1', roleId: 'role-1', hostTabId: 900 }, { tab: { id: 101 } as chrome.tabs.Tab, frameId: 7, url: 'https://gemini.google.com/app/test' })
    const deleted = await harness.invoke({ type: 'GROUP_CHAT_DELETE', chatId: 'chat-1' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('ignores stale role replies that do not match the current pending prompt', async () => {
    ...
    const stale = await harness.invoke({ type: 'TEAM_ROLE_REPLY', chatId: 'chat-1', roleId: 'role-1', messageId: 'msg-old', content: '旧初始化回复' }) as { ok: boolean; ignored?: boolean; store: WebAgentStore }
  ...
  it('does not push the switched store back to the tab that already receives the command response', async () => {
    ...
    harness.tabsSendMessage.mockClear()
    const switched = await harness.invoke({ type: 'GROUP_CHAT_SWITCH', chatId: 'chat-2' }, { tab: { id: 900 } as chrome.tabs.Tab, frameId: 0 }) as { ok: boolean; store: WebAgentStore }
  ...
  it('marks timed-out personnel errors and delivery status through TEAM_ROLE_ERROR', async () => {
    ...
    const result = await harness.invoke({ type: 'TEAM_ROLE_ERROR', chatId: 'chat-1', roleId: 'role-1', messageId: 'msg-1', reason: '人员回复超时' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('retries a visible timed-out site reply from its assistant message', async () => {
    ...
    const result = await harness.invoke({ type: 'GROUP_ROLE_RETRY_REPLY', chatId: 'chat-1', roleId: 'role-1', messageId: 'msg-timeout' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('clears chat messages and unbinds role conversations without deleting roles', async () => {
    ...
    const result = await harness.invoke({ type: 'GROUP_CHAT_CLEAR_MESSAGES', chatId: 'chat-1' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('closes a chat by clearing runtime bindings while keeping the chat data', async () => {
    ...
    const result = await harness.invoke({ type: 'GROUP_CHAT_CLOSE', chatId: 'chat-1' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('retries an interrupted role reply with a new attempt id for the pending user message', async () => {
    ...
    const result = await harness.invoke({ type: 'GROUP_ROLE_RETRY_REPLY', chatId: 'chat-1', roleId: 'role-1' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('prefers the requested retry message over a stale role prompt pointer', async () => {
    ...
    const result = await harness.invoke({ type: 'GROUP_ROLE_RETRY_REPLY', chatId: 'chat-1', roleId: 'role-1', messageId: 'msg-current' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('asks the role frame to resync the selected assistant reply and returns the updated store', async () => {
    ...
    const result = await harness.invoke({ type: 'GROUP_MESSAGE_RESYNC_REPLY', chatId: 'chat-1', roleId: 'role-1', messageId: 'msg-1' }) as { ok: boolean; store: WebAgentStore }
  ...
  it('replaces the selected assistant message when the role frame returns the complete reply', async () => {
    ...
    ) as { ok: boolean; message: { id: string; content: string; contentFormat?: string }; store: WebAgentStore }
...
function makeStore(): WebAgentStore {
  return {
    version: 1,
    chatOrder: [],
    chatsById: {},
