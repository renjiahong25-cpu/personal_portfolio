...
export function isSafeDoubaoUrl(value: string | undefined): value is string {
  if (!value || (!value.startsWith(DOUBAO_HOME_URL) && !value.startsWith('https://doubao.com/'))) return false

  try {
    const url = new URL(value)
    return url.protocol === 'https:' && (url.hostname === 'www.doubao.com' || url.hostname === 'doubao.com')
  } catch {
    return false
  }
}
...
export function isSafeSupportedChatUrl(value: string | undefined): value is string {
  return isSafeGeminiUrl(value) || isSafeChatGptUrl(value) || isSafeClaudeUrl(value) || isSafeDeepSeekUrl(value) || isSafeDoubaoUrl(value) || isSafeGrokUrl(value)
}
...
export function getDefaultChatSiteUrl(site: ChatSite | undefined): string {
  ...
  if (site === 'doubao') return DOUBAO_HOME_URL
...
export function extractSupportedConversationId(value: string | undefined): string | undefined {
  return (
    ...
    extractDoubaoConversationId(value) ??
...
export function getSupportedChatOriginForSite(value: string | undefined, site: ChatSite | undefined): string {
  ...
  if (site === 'doubao') return DOUBAO_ORIGIN
...
export function extractDoubaoConversationId(value: string | undefined): string | undefined {
  if (!isSafeDoubaoUrl(value)) return undefined
