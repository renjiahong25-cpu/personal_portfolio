...
import type { ChatSite, GroupRole, WebAgentStore, OrchestrationAutoPlanHistoryEntry, OrchestrationFlow } from './types'
...
function buildEnglishAutoOrchestrationPrompt(input: AutoOrchestrationPromptInput): string {
  return [
    'You are the Web Agent AI group-chat workflow planner.',
    ...
    '3. If existingRoles is empty, create 2-5 new people.',
     '4. Each roles[].preferredSite must be "chatgpt", "gemini", "claude", "deepseek", "doubao", or "grok"; if the user explicitly requests ChatGPT, Gemini, Claude, DeepSeek, 豆包, or Grok, assign the relevant people to that site.',
...
function summarizeExistingRole(role: GroupRole, store: WebAgentStore): Record<string, unknown> {
  return {
    id: role.id,
    name: role.name,
