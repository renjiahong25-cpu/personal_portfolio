...
import type { GroupMessage, GroupRole, WebAgentStore, RoomMode } from './types'
...
describe('group store', () => {
  ...
  it('serializes queued updates so concurrent writes are preserved', async () => {
    const addChat = (id: string) =>
      updateStoreQueued((draft: WebAgentStore) => {
...
function makeChat(id: string, messageIds: string[] = []): WebAgentStore['chatsById'][string] {
  return {
    id,
    name: id,
    mode: 'independent',
