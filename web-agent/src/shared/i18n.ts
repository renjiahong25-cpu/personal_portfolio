...
import type { WebAgentLanguage, RoleTemplate } from '../group/types'

export type TeamLanguage = WebAgentLanguage
...
const UI_TRANSLATIONS: Record<string, string> = {
  ...
  'Web Agent 导航': 'Web Agent navigation',
  '群聊': 'Chats',
  ...
  '编排': 'Orchestration',
  '免@': 'No @',
  '开启后，普通消息也会触发所有成员回复；关闭后，只有 @ 成员或 @所有人才触发回复': 'When on, plain messages ask all members to reply; when off, only @ members or @all trigger replies.',
  '笔记': 'Notes',
  '空': 'Empty',
  ...
  '输入消息；不 @ 会让所有成员回复，也可 @ 指定成员': 'Type a message; without @ all members reply, or @ specific members',
...
function translateUiPattern(source: string): string | undefined {
  ...
  const agentControlConnected = source.match(/^已连接 Web Agent CLI daemon（端口 (\d+)）。本机工具可以创建群聊并发送任务。$/)
  if (agentControlConnected) return `Connected to Web Agent CLI daemon (port ${agentControlConnected[1]}). Local tools can create chats and send tasks.`
  const agentControlConnecting = source.match(/^正在连接 Web Agent CLI daemon（端口 (\d+)）。$/)
  if (agentControlConnecting) return `Connecting to Web Agent CLI daemon (port ${agentControlConnecting[1]}).`
  const agentControlDisconnected = source.match(/^未连接 Web Agent CLI daemon（端口 (\d+)）。请安装 Web Agent CLI，或运行 web-agent daemon start 启动守护进程。$/)
  if (agentControlDisconnected) return `Not connected to Web Agent CLI daemon (port ${agentControlDisconnected[1]}). Install Web Agent CLI, or run web-agent daemon start to start the daemon.`
...
function titleCaseIdentifier(value: string): string {
  ...
  const words = withoutHan
    ...
    .split(/\s+/)
    .filter(Boolean)
  if (words.length === 0) return 'Web Agent'
