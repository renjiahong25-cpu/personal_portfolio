...
describe('extension security configuration', () => {
  ...
  it('allows extension pages to connect to the local Web Agent control daemon', () => {
    const manifest = JSON.parse(readFileSync(resolve(process.cwd(), 'public/manifest.json'), 'utf8')) as {
  ...
  it('limits iframe header overrides to supported AI chat subframes', () => {
    const rules = JSON.parse(readFileSync(resolve(process.cwd(), 'public/web-agent-frame-rules.json'), 'utf8')) as Array<{
      condition?: {
        urlFilter?: string
  ...
  it('collects nested release JavaScript chunks for compliance scanning', () => {
    const root = mkdtempSync(resolve(tmpdir(), 'web-agent-release-js-'))
    ...
    try {
      expect(collectJavaScriptFiles(root)
        .map(file => file.split(root).join('<root>').split('\\').join('/'))
        .sort()).toEqual([
