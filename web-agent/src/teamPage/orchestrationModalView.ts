...
import type { ChatSite, ExternalModelConfig, GroupChat, GroupRole, WebAgentStore, OrchestrationAutoPlanHistoryEntry, OrchestrationFlow, OrchestrationGraphSnapshot, OrchestrationStage } from '../group/types'
...
export interface OrchestrationModalDependencies {
  ...
  sendRuntimeMessage<T>(type: string, payload?: Record<string, unknown>): Promise<{ ok?: boolean; error?: string; store?: WebAgentStore; flow?: OrchestrationFlow; roles?: GroupRole[]; createdRoleIds?: string[]; reusedRoleIds?: string[]; data?: T }>
  ...
  function renderPeopleList(): void {
    ...
    for (const role of roles) {
      ...
      card.addEventListener('dragstart', event => {
        event.dataTransfer?.setData('application/x-web-agent-role-id', role.id)
      })
...
function roleModelDisplay(role: Pick<GroupRole, 'modelSource' | 'externalModelId' | 'chatSite'>, store: WebAgentStore): { label: string; className: string } {
...
function siteLabel(site: ChatSite): string {
  ...
  if (site === 'doubao') return '豆包'
  if (site === 'grok') return 'Grok'
  return 'Gemini'
}

function editableChatSites(): ChatSite[] {
  return ['deepseek', 'grok', 'doubao', 'chatgpt', 'gemini', 'claude']
}
...
function visibleChatSite(site: ChatSite): ChatSite {
  return ['gemini', 'chatgpt', 'claude', 'deepseek', 'doubao', 'grok'].includes(site) ? site : 'gemini'
}
