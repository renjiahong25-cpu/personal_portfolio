...
const FLOATING_PREFS_KEY_PREFIX = 'web-agent.orchestrationFloatingStatus.'
...
function renderMiniFlow(run: OrchestrationRun, flow: OrchestrationFlow, store: WebAgentStore, rolesById: Map<string, GroupRole>): SVGSVGElement {
  const svg = svgEl('svg')
...
function getVisibleRun(store: WebAgentStore, chatId: string): VisibleRun | undefined {
...
function stageStatusLabel(stage: OrchestrationStage, rolesById: Map<string, GroupRole>, store: WebAgentStore): string {
...
function miniNodeLines(stage: OrchestrationStage, store: WebAgentStore, rolesById: Map<string, GroupRole>, run: OrchestrationRun): string[] {
  const roleIds = stage.kind === 'review' ? stage.review?.reviewerRoleIds ?? stage.roleIds : stage.roleIds
...
function roleStatusLabel(role: GroupRole | undefined, fallbackId: string, store: WebAgentStore): string {
  if (!role) return fallbackId
  return `${role.name}（${roleModelLabel(role, store)}）`
}

function roleModelLabel(role: Pick<GroupRole, 'modelSource' | 'externalModelId' | 'chatSite'>, store: WebAgentStore): string {
