...
import type { GroupChat, WebAgentStore, RoleTemplate } from '../group/types'
...
function setupPeopleLibraryView(options: { store: WebAgentStore; templates: RoleTemplate[]; currentChat?: GroupChat; language?: WebAgentStore['settings']['language'] }) {
...
describe('team page people library view boundary', () => {
  ...
  it('shows a reload hint when the running background does not know the persona generation route', async () => {
    ...
    generatePersona.mockRejectedValueOnce(new Error('Unknown Web Agent message'))
    ...
    expect(showError).toHaveBeenCalledWith('AI 生成人设需要重新加载 Web Agent 扩展后再使用')
  })
