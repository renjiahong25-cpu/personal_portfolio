...
interface Harness {
  ...
  store: WebAgentStore
  sendRuntimeMessage: Mock<[string, Record<string, unknown>?], Promise<{ ok?: boolean; store?: WebAgentStore; flow?: OrchestrationFlow; roles?: GroupRole[]; createdRoleIds?: string[]; reusedRoleIds?: string[] }>>
...
function createHarness(): Harness {
  ...
  return {
    ...
    store,
    sendRuntimeMessage: vi.fn<[string, Record<string, unknown>?], Promise<{ ok?: boolean; store?: WebAgentStore; flow?: OrchestrationFlow; roles?: GroupRole[]; createdRoleIds?: string[]; reusedRoleIds?: string[] }>>(async () => ({ ok: true })),
...
describe('orchestration modal view', () => {
  ...
  it('keeps a user-written task when applying a built-in template', async () => {
    ...
    view.registerOrchestrationEvents()
    harness.refs.openOrchestrationEl.click()
    harness.refs.orchestrationTaskEl.value = '评估我们自己的 Web Agent 模板体验。'

    harness.refs.openOrchestrationTemplateEl.click()
    harness.refs.orchestrationTemplateContentEl.querySelector<HTMLButtonElement>('[data-template-id="parallel-merge"]')?.click()
    await flushAsync()

    expect(harness.refs.orchestrationTaskEl.value).toBe('评估我们自己的 Web Agent 模板体验。')
  })
...
function dropRole(harness: Harness, roleId: string): void {
  ...
  Object.defineProperty(drop, 'dataTransfer', { value: { getData: () => roleId, types: ['application/x-web-agent-role-id'] } })
  harness.refs.orchestrationCanvasEl.dispatchEvent(drop)
}

function storeWithRoles(baseStore: WebAgentStore, roles: GroupRole[]): WebAgentStore {
