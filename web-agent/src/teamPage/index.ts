import type { GroupChat, GroupMessage, GroupRole, WebAgentStore, RoleTemplate } from '../group/types'
...
let store: WebAgentStore = appState.store
...
function applyStore(nextStore: WebAgentStore): void {
...
function handlePrimaryModeChange(isPrimary: boolean): void {
  ...
  showError('已检测到另一个 Web Agent 页面正在运行。当前页面已暂停 AI iframe，避免两个页面同时加载导致卡死。')
}
