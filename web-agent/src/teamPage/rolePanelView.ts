...
import type { ChatSite, ExternalModelConfig, GroupRole, WebAgentStore, RoleModelSource, RoleStatus } from '../group/types'
...
function roleModelOption(role: GroupRole, store: WebAgentStore): { key: string; label: string; className: string } {
...
function roleModelKey(role: Pick<GroupRole, 'modelSource' | 'externalModelId' | 'chatSite'>, store: WebAgentStore): string {
...
function visibleChatSite(value: string | undefined): ChatSite {
  return value === 'chatgpt' || value === 'claude' || value === 'deepseek' || value === 'doubao' || value === 'grok' ? value : 'gemini'
}
