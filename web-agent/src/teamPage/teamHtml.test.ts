...
describe('team.html chat creation UI', () => {
  ...
  it('uses template default sites for library people and the add-person picker for temporary people', () => {
    ...
    expect(source).toContain("const VISIBLE_CHAT_SITES = ['gemini', 'chatgpt', 'claude', 'deepseek', 'doubao', 'grok'] as const")
  ...
  it('renders chat actions through a three-dot menu that updates, duplicates, and deletes chats', () => {
    ...
    expect(chatListSource).toContain("response.error === 'Unknown Web Agent message'")
  ...
  it('shrinks instead of closing the Web Agent window from the close control', () => {
    ...
    expect(source).not.toContain("window.confirm('确定要关闭 Web Agent 窗口吗？')")
