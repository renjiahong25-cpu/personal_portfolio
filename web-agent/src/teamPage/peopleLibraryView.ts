...
import type { ChatSite, ExternalModelConfig, GroupChat, WebAgentStore, RoleModelSource, RoleTemplate } from '../group/types'
...
const VISIBLE_CHAT_SITES = ['gemini', 'chatgpt', 'claude', 'deepseek', 'doubao', 'grok'] as const
...
export function createPeopleLibraryView(deps: PeopleLibraryViewDependencies): PeopleLibraryView {
  ...
  function defaultModelKeyForTemplate(template: RoleTemplate, store: WebAgentStore): string {
  ...
  function roleModelKey(role: { modelSource?: RoleModelSource; externalModelId?: string; chatSite?: ChatSite }, store: WebAgentStore): string {
  ...
  function selectableModels(store: WebAgentStore): Array<{ key: string; label: string; className: string }> {
    return [
...
function templateModelLabel(template: RoleTemplate, store: WebAgentStore): string {
...
function externalModels(store: WebAgentStore): ExternalModelConfig[] {
...
function personaGenerationErrorMessage(error: unknown): string {
  ...
  if (message === 'Unknown Web Agent message') return 'AI 生成人设需要重新加载 Web Agent 扩展后再使用'
  return message
}
