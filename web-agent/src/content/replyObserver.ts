...
export function createReplyObserver(options: {
  ...
  function seedStoredRoleReplies(replies: string[] | undefined): void {
    ...
    replyTracker.seedGlobal(getConversationId(), validReplies)
    log.debug('reply-history:seeded', { count: validReplies.length, conversationId: getConversationId() })
  }

  function onPromptDelivered(): void {
    replyTracker.releaseGlobalBarrier(getConversationId())
    log.debug('reply-history:barrier-released', { conversationId: getConversationId() })
  }
