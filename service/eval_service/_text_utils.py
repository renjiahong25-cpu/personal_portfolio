...
# 拒答特征词：命中其一即判定为拒答/知识库无内容。
# 注意：只保留强拒答信号，不把正常回答中的免责语（“建议咨询/请咨询/咨询专业”等）当拒答——
# 生产提示词允许正常回答携带免责句，误判会严重拉低拒答准确率与事实错误率。
_REFUSAL_WORDS = (
    "无法回答", "无法确定", "无法提供", "无法判断", "无法核实", "无法确认",
    "未找到", "未能找到", "暂无", "不提供", "不能提供", "拒绝回答", "不知道",
    "没有足够", "知识库中无", "知识库暂无", "不在服务范围", "服务范围", "不在范围",
    "不属于",
)
...
def is_refusal(text: str) -> bool:
    """判定模型输出是否为拒答/知识库无内容。
    实质性回答（含来源引用或正文较长）不视为拒答——即使末尾带免责语。"""
    if not text:
        return True
    if _CITATION_RE.search(text) or len(text) > 400:
        return False
